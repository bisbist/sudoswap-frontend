...
export const contracts = {
  ...
  factory(address, signer) {
    return new ethers.Contract(
      address || config.PairFactory.address,
      config.PairFactory.abi,
      signer || provider
    );
  },
  ...
  royaltyRouter(address, signer) {
    return new ethers.Contract(
      ...
      config.Router.royalty.abi,
      signer || provider
    );
  },
  royaltyRegistry(address, signer) {
    return new ethers.Contract(
      address || config.RoyaltyRegistry.address,
      config.RoyaltyRegistry.abi,
      signer || provider
    );
  },
};
