          ...
          onClick={async () => {
            ...
            for (let swap of swapList) {
              ...
              if (swap.nftIds.length > 0) {
                ...
                if (swap.flag) {
                  const isApprovedForAll = await nft.isApprovedForAll(
                    signerAddress,
                    router.address
                  );

                  if (!isApprovedForAll) {
                    const tnx = await nft.setApprovalForAll(
                      router.address,
                      true
                    );
                    await tnx.wait();
                  ...
                  for (let nftId of swap.nftIds) {
                    ...
                    if (operator != router.address) {
                      ...
                      if (signerAddress == nftOwner || isApprovedForAll) {
                        const txn = await nft.approve(router.address, nftId, {
                          gasLimit: 50000,
                        });
                        await txn.wait();
                      } else {
                        alert(
                          `NFT#${nftId}: ${signerAddress} is not an owner or approved for all! owner=${nftOwner}, operator=${operator}`
                        );
                        return;
                      }
                    }
            ...
            const params = {
              ...
              tokenRecipient:
                tokenRecipient != "" ? tokenRecipient : signerAddress,
              swapList: await Promise.all(
                swapList.map(async function (swap) {
                  // // prints the owner of nft
                  // const pair = contracts.pair(swap.pair)
            ...
            let txn = await router.robustSwapNFTsForToken(
              params.swapList,
              // params.minOutput,
...
export default RobustSwapNFTsForToken;
