          ...
          <tbody>
            ...
            {state.poolType != PoolType.TRADE ? (
              <AddressInput
                ...
                onChange={(assetRecipient) =>
                  setState({ ...state, assetRecipient })
                }
              />
            ) : null}
          ...
          <button
            ...
            onClick={async (event) => {
              ...
              if (params.initialNFTIDs.length > 0) {
                ...
                const notOwnedNFTIDs = initialNFTIDs.filter(
                  (nftId) => !ownedNFTIDs.find((id) => id == nftId)
                );

                if (notOwnedNFTIDs.length > 0) {
                  alert(
                    `Please remove from initial NFTs list! You don't own following NFTs: ${notOwnedNFTIDs}`
                  );
                  return;
                }
                ...
                if (
                  !(await erc721.isApprovedForAll(
                    signerAddress,
                    factory.address
                  ))
                ) {
                  if (state.setApprovalForAll) {
                    let txn = await erc721.setApprovalForAll(
                      factory.address,
                      true
                    );
                    await txn.wait();
                    ...
                    const nftIdsToApprove = approvals
                      .filter(
                        (approval) => approval.operator != factory.address
                      )
              ...
              if (params.tokenType == TokenType.ERC20) {
                ...
                const initialTokenBalance = ethers.utils.parseEther(
                ...
                if (initialTokenBalance.gt(balance)) {
                  alert(
                    `Insufficient Balance: ${ethers.utils.formatEther(
                      balance
                    )}, require: ${ethers.utils.formatEther(
                      initialTokenBalance
                    )}!`
                  );
                  return;
